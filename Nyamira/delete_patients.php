...
if ((isset($_POST['hiddenField'])) && ($_POST['hiddenField'] != "")) {
  $deleteSQL = sprintf("DELETE FROM patients WHERE userId=%s",
  ...
  $deleteGoTo = "patients_details.php";
...
$colname_delete = "-1";
if (isset($_GET['email'])) {
  $colname_delete = $_GET['email'];
}
mysql_select_db($database_nyamira_hospital, $nyamira_hospital);
$query_delete = sprintf("SELECT * FROM patients WHERE email = %s", GetSQLValueString($colname_delete, "text"));
$delete = mysql_query($query_delete, $kerugoya_hospital) or die(mysql_error());
...
<input name="hiddenField" type="hidden" id="hiddenField" value="<?php echo $row_delete['userId']; ?>"  />
 ...
 <table class="table table-hover">
  ...
  <tr>
    <td>Blood group</td>
    <td><?php echo $row_delete['blood_group']; ?></td>
  </tr>
  <tr>
    <td>Diagnosis</td>
    <td><?php echo $row_delete['diagnosis']; ?></td>
  </tr>
  <tr>
    <td colspan="2">	<div align="center">
		<input type="submit" name="submit" id="submit" value="Delete" />
       <button type="button" class="btn btn-success"> <a href="patients_details.php">Cancel</a> </button></div></td>
  </tr>
